...
#Making results_all_banch_1 reproduceable
RANDOM_SEED=42
...
tf.flags.DEFINE_float('content_embedding_scale_factor', default=1.0, help='Factor to multiply the article content embeddings, as they might significantly affect the results_all_banch_1')
...
tf.flags.DEFINE_integer('save_results_each_n_evals', default=5, help='Saves to disk and uploads to GCS (ML Engine) the incremental evaluation results_all_banch_1 each N evaluations')
...
def get_articles_features_config(acr_label_encoders):
    articles_features_config = {
        ...
        'furthest_items': {'type': 'categorical', 'dtype': 'int'},
    ...
    articles_features_config['furthest_items']['cardinality'] = articles_features_config['article_id']['cardinality']
...
def process_articles_metadata(articles_metadata_df, articles_features_config):
    articles_metadata = {}
    for feature_name in articles_features_config:
        if feature_name is 'furthest_items':
            continue
...
def get_session_features_config(nar_label_encoders_dict):
    session_features_config = {
        ...
        'sequence_features': {
            ...
            'furthest_items': {'type': 'categorical', 'dtype': 'int'},#, 'cardinality': 72933},
...
def main(unused_argv):
    try:
        ...
        for chunk_id in range(0, len(training_files_chunks)-1):
            start_one_iteration = time()
            print(f"Iterations progress : {chunk_id}/{(len(training_files_chunks)-1)}")
            ...
            input_lambda = lambda: prepare_dataset_iterator(training_files_chunk, session_features_config,
                                                       batch_size=FLAGS.batch_size,
                                                       truncate_session_length=FLAGS.truncate_session_length)
            model.train(input_fn=input_lambda)
            ...
            if chunk_id % FLAGS.save_results_each_n_evals == 0:
                ...
                #If must persist and load model ouput in a local cache (to speedup in ML Engine)
                if FLAGS.use_local_cache_model_dir:
                    tf.logging.info('Uploading cached results_all_banch_1 to GCS')
                    upload_model_output_to_gcs(model_output_dir, gcs_model_dir=gcs_model_output_dir,  
                                               #files_pattern=None)
                                               files_pattern=[#'events.out.tfevents.', 
                                               '.csv', '.json'])

            end_one_iteration = time()
            total_one_iteration_time = (end_one_iteration - start_one_iteration)/60
            print(f"One iteration took {total_one_iteration_time} minutes")
            print(f"Total expected time to train and eval : {(len(training_files_chunks)-1)*total_one_iteration_time/60} hours")
