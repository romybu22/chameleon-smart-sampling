...
session_id_to_furthest_articles = {}
...
def load_acr_module_resources(acr_module_resources_path):
    ...
    return get_article_text_length, content_article_embeddings
...
def load_sessions_hours(folder_path):
    ...
    hour_folders = sorted([path for path in os.listdir(folder_path) \
                             if (os.path.isdir(os.path.join(folder_path,path))
                                 and int("{:0>5}".format(path.split('=')[1]))>=start_hour
                                 and int("{:0>5}".format(path.split('=')[1]))<=end_hour)],
...
def get_closest_items(row, num_of_articles_in_sub_group, content_article_embeddings):

    session_articles = row["article_id"]
    session_articles_embeddings = content_article_embeddings[session_articles, :]
    session_avg_embeddings = session_articles_embeddings.mean(axis=0)

    distances = []
    for idx, row in enumerate(content_article_embeddings):
        distances.append((idx, np.linalg.norm(row - session_avg_embeddings)))

    # distances = [(i, np.linalg.norm(content_article_embeddings[i] - session_avg_embeddings))
    #              for i in range(content_article_embeddings.shape[0])]


    distances.sort(key=lambda tup: tup[1])

    # 0 till num_of_articles_in_sub_group are the closest (hardest samples that won't be filter)
    #distances = distances[num_of_articles_in_sub_group:len(distances)-1]
    distances = distances[0:num_of_articles_in_sub_group]

    distances = [tup[0] for tup in distances]

    # for session_article in session_articles:
    #     if session_article not in distances:
    #         distances.append(session_article)

    for session_article in session_articles:
        if session_article in distances:
            distances.remove(session_article)

    return distances


def make_sequence_example(row, num_of_articles_in_sub_group, content_article_embeddings):
    context_features = {
    ...
    sequence_features = {
        ...
        'furthest_items': make_sequential_feature(get_closest_items(row, num_of_articles_in_sub_group,
                                                                    content_article_embeddings)),
    ...
    example = tf.train.SequenceExample(feature_lists=sequence_feature_lists, context=context)

    return example

def export_sessions_hour_to_tf_records(hour_index, sessions_df, output_path, num_of_articles_in_sub_group,#478417
                                       content_article_embeddings):
    ...
    save_rows_to_tf_record_file(map(lambda x: x[1], sessions_df.iterrows()),
                            make_sequence_example, len(sessions_df),
                            export_filename=export_file_template.format(hour_index),
                            content_article_embeddings=content_article_embeddings,
                            num_of_articles_in_sub_group=num_of_articles_in_sub_group)
...
def main():
    ...
    # args = argparse.Namespace(input_sessions_json_folder_path='C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_inputs\\pickles\\acr_articles_metadata_embeddings.pickle',
    #                           input_acr_metadata_embeddings_path='C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_inputs\\pickles\\nar_encoders_adressa.pickle',
    #                           input_nar_encoders_dict_path='C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_inputs\\sessions_processed_by_spark\\1_week',
    #                           number_hours_to_preprocess=1,
    #                           output_nar_preprocessing_resources_path='C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_outputs\\smart_sampling\\pickles/nar_preprocessing_resources.pickle',
    #                           output_sessions_tfrecords_path='C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_outputs\\smart_sampling\\sessions_tfrecords_by_hour/adressa_sessions_hour_*.tfrecord.gz' )
    # 'input_sessions_json_folder_path'
    # 'C:\\Users\\operi157093\\Desktop\\Master\\RecSys_project\\nar\\2_preprocess\\nar_preprocess_inputs\\pickles\\acr_articles_metadata_embeddings.pickle'
    #
    ...
    get_article_text_length_fn, content_article_embeddings = load_acr_module_resources(args.input_acr_metadata_embeddings_path)
    #get_article_text_length_fn = None
    total_embeddings = content_article_embeddings.shape[0]
    in_loop_embeddings = 0
    ...
    num_of_articles_in_sub_group = 1000
    #96-119
    total_sessions = 0
    for (hour_index, sessions_hour) in load_sessions_hours(args.input_sessions_json_folder_path):        
        ...
        in_loop_embeddings += len(sessions_hour)
        ...
        total_sessions += len(sessions_hour_df)

        #stats.append(hour_stats)
        
        clicks_by_articles_counters.append(hour_clicks_by_articles_counter)

        num_of_articles_in_sub_group = int(num_of_articles_in_sub_group*0.995)
