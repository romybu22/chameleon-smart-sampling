...
class NARModuleModel():
    ...
    def __init__(self, mode, inputs, labels,  
        ...
        with tf.variable_scope("main", initializer=contrib.layers.xavier_initializer()):
            ...
            with tf.variable_scope("neg_samples"):
                ...
                # #total_articles = 10310
                # all_articles_ids_array = np.reshape(np.array(range(73300, total_articles)), (1, total_articles-73300))
                ...
                closest_items_reshape = tf.reshape(closest_items, [-1])
                #furthest_items, _ = tf.setdiff1d(articles_ids_tensor, closest_items_reshape, index_dtype=tf.int64)

                batch_negative_items = self.get_batch_negative_samples(all_clicked_items, closest_items_reshape,
    ...
    def la(self, sample):
        la = tf.cond(
            tf.math.greater_equal(tf.norm(self.next_item_label - sample, ord='euclidean'),
                                  tf.constant(0.0001, tf.float64)),
            sample,
            tf.constant(0, tf.int64))
        return  la

    def smart_choice(self, valid_samples_session, closest_items):

        valid_samples_session_dis = tf.map_fn(lambda sample: self.la(sample, closest_items),
                                              valid_samples_session)

        valid_samples_session_dis_nonzero = tf.boolean_mask(valid_samples_session_dis,
                                                            tf.cast(tf.sign(valid_samples_session_dis), tf.bool))

        return valid_samples_session_dis_nonzero

    def get_neg_items_click(self, valid_samples_session, num_neg_samples, closest_items, session_item_ids):
        #Shuffles neg. samples for each click

        global SMART_SAMPLING
        # Ignoring negative samples clicked within the session (keeps the order and repetition of candidate_samples)
        if SMART_SAMPLING:
            # valid_samples_session, _ = tf.setdiff1d(candidate_samples, tf.reshape(furthest_items, [-1]), index_dtype=tf.int64)
            # valid_samples_session  = tf.sets.set_intersection(valid_samples_session[None,:],closest_items[None,:])
            # valid_samples_session = tf.sparse.to_dense(valid_samples_session)
            # valid_samples_session = tf.reshape(valid_samples_session, [-1])

            # concat = tf.concat(valid_samples_session, closest_items, index_dtype=tf.int64)
            # c_d = concat- tf.setdiff1d(valid_samples_session, closest_items, index_dtype=tf.int64)
            # valid_samples_session = c_d
            #valid_samples_session = self.smart_choice(valid_samples_session, closest_items)

            disju, _ = tf.setdiff1d(valid_samples_session, closest_items,
                                                    index_dtype=tf.int64)

            intersection , _ = tf.setdiff1d(valid_samples_session, disju,
                                                    index_dtype=tf.int64)
            valid_samples_session = intersection
    ...
    def get_neg_items_session(self, session_item_ids, closest_items, candidate_samples, num_neg_samples):
        valid_samples_session, _ = tf.setdiff1d(candidate_samples, session_item_ids, index_dtype=tf.int64)
        #real_ , _ = tf.setdiff1d(candidate_samples, valid_samples_session, index_dtype=tf.int64)
        #valid_samples_session = candidate_samples
        ...
        session_clicks_neg_items = tf.map_fn(lambda click_id: tf.cond(tf.equal(click_id, tf.constant(0, tf.int64)), 
                                                                      ...
                                                                      lambda: self.get_neg_items_click(valid_samples_session, num_neg_samples, closest_items, session_item_ids)
                                                                      )
                                             , session_item_ids)                                                     
    ...
    def get_negative_samples(self, all_clicked_items, closest_items, candidate_samples, num_neg_samples):
        with tf.variable_scope("negative_samples"):

            # candidate_samples = tf.map_fn(lambda furthest_item: self.filter_furthest_samples(furthest_item,
            #                                                                                  candidate_samples)
            #                               , furthest_items)
    ...
    def get_batch_negative_samples(self, all_clicked_items, closest_items, additional_samples, num_negative_samples,
        ...
        with tf.variable_scope("neg_samples_batch"):
            ...
            batch_negative_items = self.get_negative_samples(all_clicked_items, closest_items,
...
class ItemsStateUpdaterHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):     
        ...
        if self.mode == tf.estimator.ModeKeys.EVAL:
            self.eval_streaming_metrics_last = {}
            #self.eval_streaming_metrics_last['hitrate_at_1'] = run_values.results_all_banch_1['hitrate_at_1']
            ...
            #self.eval_streaming_metrics_last['ndcg_at_n'] = run_values.results_all_banch_1['ndcg_at_n']
